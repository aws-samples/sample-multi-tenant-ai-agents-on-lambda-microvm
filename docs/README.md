...
The "why" behind the code in [`../iac/`](../iac/): the design decisions made while building.
...
| [iac-tooling-support.md](design/iac-tooling-support.md) | IaC tooling matrix: CFN has native `AWS::Lambda::MicrovmImage`/`NetworkConnector` (CDK L1 only; SAM/Terraform-aws lag) — why `iac/` is CFN-native |
